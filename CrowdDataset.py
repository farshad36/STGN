...
class TestDataset(Dataset):
    ...
    def load_example(self, img_f):
        ...
        img = cv2.imread(os.path.join(self.img_path, img_name))#(os.path.join(self.img_path, dir_name, img_name))
#         if len(img.shape) == 2:
#             print(os.path.join(self.img_path, dir_name, img_name))
        points = np.load('D:\\univercity\\article\\mall_dataset\\Mall\\labels.npy')#np.load(os.path.join(self.label_path, dir_name, img_name[:-4] + '.npy'))
        ...
        if H_orig != self.out_shape[0] or W_orig != self.out_shape[1]:
            ...
            img = cv2.resize(
                img, (self.out_shape[1], self.out_shape[0]), cv2.INTER_LINEAR)
...
#         if len(img.shape) == 2:
#             print(os.path.join(self.img_path, dir_name, img_name))
        points = np.load('D:\\univercity\\article\\mall_dataset\\Mall\\labels.npy')
        H_orig, W_orig = img.shape[:2]
        if H_orig != self.out_shape[0] or W_orig != self.out_shape[1]:
            # img = img.resize((self.out_shape[1], self.out_shape[0]), Image.BILINEAR)
            img = cv2.resize(
        ...
        density = gaussian_filter_density(
            gt, self.gamma, self.k, self.adaptive)
        ...
        gt = gt[:, :, np.newaxis].astype('float32')
        np.savetxt('./nparray/test.out', img, delimiter=',')
...
if __name__ == '__main__':
    ...
    if dataset == 'UCSD':
        ...
        path = 'D:\\mine\\article\\mall_dataset\\Mall'
    elif dataset == 'FDST':
        path = './FDST/FDST'
