...
def main():
    ...
    dataset_path = os.path.join(
        'D:\\univercity\\article\\mall_dataset', args['dataset'])
    ...
    for epoch in range(args['epochs']):
        ...
        log = '{} density loss: {:.3f} | count loss: {:.3f} | count error: {:.3f}'.format(
            datetime.now(), train_density_loss, train_count_loss, train_count_err)
        ...
        log = '{} density loss: {:.3f} | count loss: {:.3f} | MAE: {:.3f} | MSE: {:.3f}'.format(
            datetime.now(), valid_density_loss, valid_count_loss, valid_mae, valid_mse)
        print(log, flush=True)
        with open(log_path, 'w') as f:
            f.write(log + '\n')
